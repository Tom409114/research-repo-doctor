...
def _record(*, version: str = "v0.2.21", doi: str = "10.5281/zenodo.21282779") -> dict:
    return {
        "doi": doi,
        ...
        "metadata": {
            ...
            "related_identifiers": [
                {"identifier": "https://github.com/Tom409114/research-repo-doctor/tree/v0.2.21"}
            ],
        },
        "files": [{"key": "Tom409114/research-repo-doctor-v0.2.21.zip"}],
    }
...
def test_zenodo_metadata_accepts_current_release_and_warns_about_generic_creator() -> None:
    ...
    result = script.validate_record(_record(), "0.2.21", "10.5281/zenodo.21282779")

    assert result.ok is True
    assert "matches rrdoctor 0.2.21" in result.message
...
def test_zenodo_metadata_rejects_stale_release_version() -> None:
    ...
    result = script.validate_record(_record(version="v0.2.3"), "0.2.21", "10.5281/zenodo.21282779")
...
def test_zenodo_metadata_rejects_wrong_release_doi() -> None:
    ...
    result = script.validate_record(
        ...
        "0.2.21",
        "10.5281/zenodo.21282779",
    )

    assert result.ok is False
    assert "expected 10.5281/zenodo.21282779" in result.message
